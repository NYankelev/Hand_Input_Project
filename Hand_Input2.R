...
dest_folder <- "C:/Users/nated/Desktop/Data Science/"
...
for (i in 1:file_num) {  
  ...
  if (ncol(p14) %in% c(14,15,16)) {
    
    p14 <- p14[,(ncol(p14)-12):ncol(p14)] # Guarantees we return only the data in the standard case of data in 13 cols.
    
  }
  
  if (ncol(p14)>22) {
    
    p14 <- p14[,(ncol(p14)-20):ncol(p14)] # Guarantees we return only the data when "XXX" is split from other cols.
    p14 <- p14[,c(2,4,6,8,9,10,12,13,14,15,17,19,21)]
  }
  
  # This next one is if XXX is on same column as data, but for some reason erroneous columns have been created.
  # Currently the manner in which this happens is unpredictable, and I think the best procedure is to create a separate
  # output where this sort of thing happened and then just loop back in earlier without printing p14. 
  
  if ((ncol(p14)>16) & (ncol(p14)<22)) {
    
    cat("Hand Input #",i,"\n",file = paste(dest_folder,"HInputerror.csv",sep=""),sep="", append = TRUE)
    write.table(naic_row,file = paste(dest_folder,"HInputerror.csv",sep=""),sep=",",row.names=FALSE, col.names = FALSE, append = TRUE)
    write.table(p2,file = paste(dest_folder,"HInputerror.csv",sep=""), sep=",", col.names = NA, row.names = TRUE, append = TRUE)
    write.table(p3,file = paste(dest_folder,"HInputerror.csv",sep=""), sep=",", col.names = NA, row.names = TRUE, append = TRUE)
    write.table(p4,file = paste(dest_folder,"HInputerror.csv",sep=""), sep=",", col.names = NA, row.names = TRUE, append = TRUE)
    write.table(p5,file = paste(dest_folder,"HInputerror.csv",sep=""), sep=",", col.names = NA, row.names = TRUE, append = TRUE)
    cat("\n",file = paste(dest_folder,"HInputerror.csv",sep=""),sep="", append = TRUE)
    
    naic_bad <- rbind(naic_bad,naic_num)
    next
  ...
  naic_good <- rbind(naic_good, naic_num)
  
}

names(naic_good) <- "No_Errors"
names(naic_bad) <- "P14_Not_Printed_Due_to_Errors"
